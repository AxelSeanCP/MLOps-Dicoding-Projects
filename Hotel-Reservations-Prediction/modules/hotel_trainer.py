...
def get_model(hparams, show_summary=True):
    ...
    deep = tf.keras.layers.Dense(hparams.get('units'), activation="relu")(concatenate)

    for _ in range(int(hparams.get('num_layers'))):
        deep = tf.keras.layers.Dense(hparams.get('units'), activation='relu')(deep)
